...
def main():
    ...
    # Benchmark logging in memory (do not print per-frame)
    benchmark_data = []
    detailed_data  = []

    if ENABLE_BENCHMARK:
        print(f"[BENCHMARK] Starting {BENCHMARK_FRAMES}-frame benchmark harness (logging to memory)...")
    else:
    ...
    while True:
        ...
        if ENABLE_BENCHMARK:
            ...
            detailed_data.append({
                "cam_read_ms":       t_cam_ms,
                "color_conv_ms":     t_conv_ms,
                "draw_display_ms":   t_draw_ms,
                "gesture_cursor_ms": t_gesture_ms,
                "cap_read_ms":       stream.get_last_cap_read_ms()
            })
    ...
    if ENABLE_BENCHMARK:
        print_summary_table(benchmark_data, detailed_data, backend_info)
